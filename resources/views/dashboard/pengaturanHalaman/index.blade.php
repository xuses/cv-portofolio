...
@section('konten')
    <form action="{{ route('pengaturanHalaman.update')}}" method="POST">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select class="form-control form-control-sm" name="_halaman_Penghargaan">
                    <option value="">-pilih-</option>
                    @foreach ($datahalaman as $item)
                        <option value="{{ $item->id }}" {{ get_meta_value('_halaman_Penghargaan')==$item->id?'selected':''}}>{{ $item->judul}}</option>
                    @endforeach
                </select>
            </div>
        </div>
